...
const SendEmailPage = () => {
  ...
  const token = cookie.get("userLoggedES") as string | undefined;
  ...
  const { data: recipientLists, isLoading: isLoadingLists } =
    useGetRecipientListsQuery();
  const { data } = useGetSenderFromQuery();

  const senderEmails: SenderEmail[] = (data?.sender_emails || []).map(
    (email: string) => ({
      label: email,
      value: email,
    })
  ...
  const onSubmit = async (data: TSendEmail) => {
    try {
      ...
      if (data.schedule) {
        ...
        if (difference <= 0) {
          toast.error(t("futureTimeError"), { duration: 4000 });
          return;
        }
      ...
      if (delaySeconds) formData.append("delay", `${delaySeconds}`);
      ...
      if (attachmentFile) formData.append("attachment_folder", attachmentFile);
      ...
      const error = err as AxiosError<{ مستلمين_بمعلومات_ناقصة?: any[] }>;
  ...
  if (isLoadingLists || LoadingEmail) {
    return (
        ...
        <div className="flex flex-col items-start justify-center space-y-5 mb-10">
          ...
          <div className="w-full flex flex-col items-center justify-between space-y-8">
            <div className="w-full flex flex-col items-center justify-between">
              ...
              <div className="w-full flex flex-col items-center gap-3">
                <div
                  className={`w-full flex items-start gap-2 border border-accent rounded-lg`}
                >
                  <div
                    className={`bg-accent p-2 ${
                      currentLng === "ar" ? "rounded-l-md" : "rounded-r-md"
              ...
              <div className="w-full flex flex-col items-center gap-3">
                <div className="w-full flex flex-col items-center gap-3">
                  ...
                  >
                    ...
                    >
                      <SoUserGroup2 className="h-6 w-6 text-primary" />
                    </div>
                    <Dropdown
                      value={selectedList}
                      onChange={(e) => setSelectedList(e.value)}
                      options={recipientLists as RecipientList[]}
                      optionLabel="name"
                      itemTemplate={(list: RecipientList) => (
                        <span className="w-full flex justify-between items-center">
                          {list?.name}{" "}
                          <span className="flex items-center gap-1">
                            <SoUserGroup className="w-5 h-5 " />(
                            {list.recipients.length})
                          </span>
                        </span>
                      )}
                      optionValue="id"
                      placeholder={t("selectListPlaceholder")}
                ...
                <div className="w-full flex flex-col items-center gap-3">
                    ...
                    <Controller
                      ...
                      render={({ field }) => (
                        <Select
                          ...
                          options={
                            selectedList
                              ? (recipientLists as RecipientList[]).find(
                          ...
                          getOptionValue={(option: Recipient) => option.id}
                          value={(recipientLists as RecipientList[])
                            .flatMap((list) => list.recipients)
                            .filter((recipient) =>
                              field.value.includes(recipient.id)
                            )}
                          onChange={(selected) =>
                            field.onChange(
                              selected.map((option: Recipient) => option.id)
                            )
                          }
                  ...
                  <div className="w-full flex items-end">
                    <InputErrorMessage msg={errors.recipient_ids?.message} />
                  </div>
                </div>
              </div>
              ...
              <div className="w-full flex flex-col items-center gap-3">
                ...
                >
                  ...
                  <Dropdown
                    ...
                    options={EmailTemplate as EmailTemplate[]}
                    optionLabel="name"
                    optionValue="id"
              ...
              <div className="w-full flex flex-col items-center gap-3 mb-2">
                ...
                >
                  ...
                  <div className="w-full flex flex-col items-center gap-3">
                    <div className="w-full flex flex-row items-center gap-2 rounded-lg">
                      <div className="relative w-full">
                        <input
                          type="file"
                          accept=".zip"
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <Select
                        ...
                        getOptionLabel={(option: SenderEmail) => option.label}
                        getOptionValue={(option: SenderEmail) => option.value}
                        value={senderEmails.find(
                          (option: SenderEmail) => option.value === field.value
                        )}
                        onChange={(selected) =>
              ...
              <div className="w-full flex flex-col items-center gap-3 mt-2">
                <div className="w-full">
                  <button
                    ...
                    className={`flex w-full justify-between gap-12 border-b border-[#D5D5D5] bg-transparent text-sm font-medium p-3 pb-4 focus:outline-none focus-visible:ring focus-visible:ring-primary ${
                      activeTab === "survey" ? "text-primary" : "text-[#212121]"
                    }`}
                  >
                ...
                <div className="w-full">
                  <button
                    ...
                    className={cn(
                      "w-full flex flex-row justify-between gap-12 border-b border-[#D5D5D5] bg-transparent text-sm font-medium p-3 pb-4 focus:outline-none focus-visible:ring focus-visible:ring-primary",
                      activeTab === "custom" ? "text-primary" : "text-[#212121]"
                    )}
                  >
                    <span className="text-base font-medium max-sm:text-base duration-150">
                      {t("customSurvey")}
